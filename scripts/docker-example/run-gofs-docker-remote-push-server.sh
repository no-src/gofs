...
docker run -it --rm -v "$PWD":"$WORKDIR" -p 8105:8105 -p 443:443 --name running-gofs-remote-push-server nosrc/gofs:latest \
  gofs -source="rs://0.0.0.0:8105?mode=server&local_sync_disabled=true&path=$WORKDIR/remote-push-server/source&fs_server=https://$GOFS_SERVER_ADDR" -dest="$WORKDIR/remote-push-server/dest" -users="gofs|password|rw" -tls_cert_file=cert.pem -tls_key_file=key.pem -push_server
